var announcement = (function(window) {
  ...
  return (function() {
    ...
    function init(options) {
      //dom 不存在
      if (
        domNotExist($(".announcement-panel")) ||
        domNotExist($(".announcement-content"))
      ) {
      ...
      //beforeLoad
      typeof options.beforeLoad === "function"
        ? options.beforeLoad()
        : undefined;
      ...
      var content = `<a href="#"><img src="${options.imgPath}/close.png"></a><div class="title-wrapper"><h1 class="headTitle">${options
        ...
        .pubdate}</span>发布单位：<span>${options.data.dataMap
        .puborgid}</span></div></div><pre>${options.data.dataMap
        .content}</pre>`;
      ...
      if (options.data.listDataMap.length > 0) {
        options.data.listDataMap.forEach(function(item) {
            ...
            var fileType = item.filename.includes("doc")
                ? "word"
                ...
                : item.filename.includes("ppt")
                    ...
                    ? "pdf"
                    : item.filename.includes("txt") ? "txt" : "unknown";
      ...
      options.backgroundColor
        ? colorValid(options.backgroundColor) === null
          ? reportError("颜色格式不正确，仅仅支持十六进制格式！")
      ...
      //设置z-index
      options.zIndex
        ? myIsNaN(parseInt(options.zIndex, 10))
          ? reportError("z-index不合法！")
          : $(".announcement-panel").css({
    ...
    function initEvents() {
      //面板关闭
      $(
        ".announcement-panel .announcement-content >a >img"
      ).on("click", function(e) {
      ...
      $(".appendix").on("click", function(e) {
        //判断方法是否undefined
        typeof opts.fileDownload === "function"
          ? opts.fileDownload(
              $(this)
                .find(".appendix-icon>a")
                .attr("value")
            )
          : undefined;
      });
